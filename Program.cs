    ...
    public class Todo
    {
        ...
        public static void SaveToFile(string input)
        {
            ...
            if (inputArray.Length <= 1)            {
                Console.WriteLine("You have to enter a file name");
                return;
            }
            fileName = inputArray[1];
            string todoFileName = fileName;
            Console.WriteLine($"Saving changes to file {todoFileName} ...");

            using (StreamWriter sw = (File.Exists(fileName)) ? new StreamWriter(fileName) : File.CreateText(fileName))
            {
                foreach (TodoItem item in list)
                {
